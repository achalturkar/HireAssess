...
export default function RolesPage() {
  ...
  useEffect(() => {
    if (isSuperAdmin) {
      // listCompanyOptions now takes the access token as its first
      // argument (same authFetch(path, token, init) pattern users.ts
      // uses everywhere else) — the old listCompanies() call here had no
      // token param at all, which no longer matches the real export.
